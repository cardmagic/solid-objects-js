...
export class SolidObjectsRuntime {
  ...
  async findBy(input: {
    ...
    if (!message) return await this.absentMessage(input)
  ...
  // An actor remembers the idempotency keys of its own finished turns, so a
  // key it remembers and no longer has a row for was pruned. A request id is
  // generated by the runtime, so no actor remembers one and both cases read
  // the same. A caller the policy refuses learns nothing either way.
  private async absentMessage(input: {
    reference?: ActorReferenceCore<Actor>
    idempotencyKey?: string
    authorizationContext?: AdministrationOptions["authorizationContext"]
  }): Promise<undefined> {
