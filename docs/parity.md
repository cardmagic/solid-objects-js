...
| Result recovery and sync timeout diagnostics                                                                  | Native  | Status, result, and wait reauthorize the stored operation; whole-call adapter deadlines distinguish uncommitted enqueue from recoverable wait timeout and report database, activation, and mailbox blockers. |
...
| Capability               | Status | TypeScript shape or remaining work                                                                                                                                    |
| ------------------------ | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SQLite                   | Native | Uses built-in `node:sqlite`, serialized process-local access, foreign keys, strict tables, database time, and deadline-bounded access and lock waits.                 |
