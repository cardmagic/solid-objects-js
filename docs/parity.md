...
| One-shot and recurring reminders                                                                              | Partial | Scheduling, catch-up policy, stale-claim recovery, and invalid-operation pausing exist. Add replacement instrumentation and operational inspection/resume.                                                    |
...
| Committed snapshots                                                                                           | Partial | Authorized state snapshots exist. Add instance ID, revision, and explicit observable reads without creating mailbox history.                                                                                  |
...
| Dead-letter inspection and retry                                              | Planned | Add deny-by-default administration APIs and idempotent retry linkage.                                                              |
| Reconciliation reads                                                          | Planned | Add active, quiet-without-work, batched-state, and application-owner reconciliation primitives without exposing mutable state.     |
| Message, process, and opt-in instance retention                               | Planned | Add preview-first bounded pruning and preserve live mailbox/outbox/dead-letter dependencies.                                       |
| Doctor and schema verification                                                | Planned | Add structured checks for configuration, schema shape, database version, live roles, authorization posture, and a real round trip. |
...
| Action Cable channels and signed stream names                | Not applicable | Provide an authenticated, transport-neutral subscription protocol instead of Action Cable.                                                       |
| Authorized subscriptions                                     | Planned        | Add deny-by-default subscription policy, signed opaque subscription tokens, reconnect replay, and per-delivery revision fencing.                 |
