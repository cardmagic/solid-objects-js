...
| Direct application-write isolation during actor code                                                          | Partial | `guardApplicationDatabase()` fails closed for direct writes during actor execution, while registered commit actions remain writable. Unwrapped ORM pools and third-party clients cannot be intercepted. |
